...
class LimeBase(object):
    ...
    def explain_instance_with_data(
        ...
        unsorted_weights = zip(used_features, easy_model.coef_)
        ...
        sorted_txt = sorted(
            zip(used_features[:n_txt_features], easy_model.coef_),
            key=lambda x: np.abs(x[1]),
            reverse=True,
        )
        ...
        sorted_img = sorted(
            zip(
                used_features[n_txt_features : (n_img_features + n_txt_features)],
                easy_model.coef_,
            ),
            key=lambda x: np.abs(x[1]),
            reverse=True,
        )
        for item in sorted_img:
            item = list(item)
            item[0] -= n_txt_features
            item = tuple(item)
        ...
        if n_det_features == 0:
            ...
            sorted_det = sorted(
                zip(used_features[n_txt_features + n_img_features :], easy_model.coef_),
                key=lambda x: np.abs(x[1]),
                reverse=True,
            )
